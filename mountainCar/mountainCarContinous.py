...
def neural_network_model(input_size):
    ...
    network = fully_connected(network, 2, activation='softmax')  # this is linked with output array size
...
for each_game in range(10):
    ...
    for i in range(goal_steps):
        ...
        if len(prev_obs) == 0:
            ...
            # if env.step([0])[0][1] > 0:
            #     action = np.argmin(model.predict(prev_obs.reshape(-1, len(prev_obs), 1)))
            #     # action = 1
            # else:
            #     action = -1
            # # print prev_obs
